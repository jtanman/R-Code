...
g5_assumptions <- read.csv('burhan_optimistic_spreadoutspend.csv')
...
newdata <- tibble(install_date = dates) %>%
  ...
  select(install_date, month, !!! group_variables, installs_new, total_spend_new, cpi)

installs <- cohort_simulation %>% select(!!! group_variables, type) %>% distinct() %>% arrange(type) %>%
  mutate(installs = c(1.1e6, 1.35e6, 610000))
newdata <- merge(tibble(install_date = dates), installs) %>%
  mutate(
    month = as.factor(format(install_date, format='%Y-%m'))
  ) %>%
  group_by(type) %>%
  ...
  select(install_date, month, !!! group_variables, installs_new)

name <- 'burhan_optimistic_spreadoutspend'
