...
cohort_noplatform <- readRDS('miso_beta_adjusted_revenue_modeling_cohortday_noplatform.RData')
cohort_noplatform_weeks <- readRDS('miso_beta_rpdau_arpu_adjusted_organic_1_revenue_modeling_cohortday_noplatform_weeks.RData')
...
if(install_platform_granularity){
  ...
  group_variables <- quos(campaign_type, install_platform)
}else{
  cohort_simulation <- cohort_noplatform
  # group_variables <- quos(campaign_type, dup_account)
  group_variables <- quos(campaign_type)
}
...
filename <- 'miso_projections_10_22'
...
dates <- seq(as.Date('2018-10-24'), by=1, length.out=length(unique(spenddata$install_date)))
...
# ggplot(predictdata, aes(date, cumrev)) +
#   geom_line() +
#   labs(title='Cumulative Revenue MISO/NISO Beta Adjusted Prediction', subtitle=paste0('Total Rev: ', format.money(sum(predictdata$revenue), 0)), caption='Organic ARPI ratio adjusted to 1:1')
...
# write.csv(select(predictdata, date, campaign_type, installs, revenue, revenue_rpdau, rpdau, arprpdau, cumrev), 'miso_rev_by_type_new_installs.csv', row.names = FALSE)

# ggplot(predictdata, aes(date, cumrev, fill=campaign_type)) +
#   geom_area() +
#   labs(title='Cumulative Revenue MISO/NISO Beta Adjusted Prediction', subtitle=paste0('Total Rev: ', format.money(sum(predictdata$revenue), 0)))
